...
├── app/
│   ├── __init__.py        # 包初始化
│   ├── config.py          # 运行时配置（环境变量、Prompt 再导出）
│   ├── container.py       # 依赖注入容器
│   ├── errors.py          # 业务异常扩展
│   ├── game_context.py    # 游戏上下文对象
...
│   ├── api/               # API 路由层
│   │   ├── __init__.py
│   │   ├── game_routes.py       # 游戏核心路由（聊天流、记忆、游戏管理、故事扩展）
│   │   ├── character_routes.py  # 角色管理路由（角色、关系、NPC生成、对话）
│   │   ├── player_routes.py     # 玩家角色路由（创建、生成、属性、技能）
│   │   ├── novel_routes.py      # 小说生成路由（增量规划、章节、进度）
│   │   ├── save_routes.py       # 存档管理路由（存档、历史、回退、导入导出）
│   │   ├── check_routes.py      # 骰子检定路由（检定、信息查询）
│   │   └── item_routes.py       # 道具系统路由（使用、装备）
...
│       ├── memory_utils.py      # 记忆文档工具
│       └── history_round_count.py # 历史轮次计数
├── games/                 # 游戏数据目录
...
└── scripts/               # 工具脚本
    ...
    ├── run_real_e2e_test.sh     # E2E 测试脚本
    └── run_regression_tests.sh  # 回归测试脚本
```
...
#### POST /api/characters/{char_id}/avatar
为指定角色生成 AI 头像
...
#### POST /api/novel/incremental
增量规划并生成小说章节（标题、章节大纲、自动撰写）

**请求体**:
```json
{
  "ending_type": "好结局",
  "custom_description": "自定义结局描述（可选）",
  "current_round": 0
}
```
...
#### GET /api/novel/progress
获取小说生成进度

#### GET /api/novel/content
获取已生成的小说全文

#### DELETE /api/novel/reset
重置小说生成状态
...
#### GET /api/check/info
获取当前玩家检定信息（可用属性、技能列表等）

### 道具系统

#### POST /api/item/use/{item_id}
使用指定道具

#### POST /api/item/equip/{item_id}
装备/卸下指定道具
...
- `NPC_LIST_GENERATION_PROMPT` / `NPC_DETAIL_GENERATION_PROMPT` - NPC 名录生成 / NPC 详细设定生成模板
- `CHARACTER_LIST_GENERATION_PROMPT` / `CHARACTER_DETAIL_GENERATION_PROMPT` - 通用角色名录生成 / 角色详细设定生成模板
- `RELATION_GENERATION_PROMPT` - 角色关系生成模板
- `NOVEL_GENERATION_PROMPT` / `NOVEL_TITLE_PROMPT` / `NOVEL_CHAPTER_PROMPT` / `NOVEL_ENDING_PROMPT` / `NOVEL_INCREMENTAL_PLAN_PROMPT` - 小说生成相关模板（全篇 / 标题规划 / 章节 / 终章 / 增量续写）
...
6. **并发与速率**: 高并发 LLM 调用仍可能受提供商 Rate Limit 影响

## 未来改进方向

1. **数据库支持**: 迁移到 SQLite 或 PostgreSQL 提升查询性能
2. **WebSocket**: 在现有 SSE 基础上，可选支持 WebSocket 实时流式响应
3. **用户系统**: 添加多用户支持和权限管理
4. **导入导出**: 支持游戏数据打包导出和导入
5. **多语言**: 支持 English 等多语言界面
6. **移动端适配**: 优化移动设备交互体验
7. **剧情分支图**: 可视化展示故事走向和选择分支
8. **存档对比**: 显示不同存档点的差异对比
...
- **网关鲁棒重塑**：上线含指数退避（Exponential Backoff）重试机制的 LLM 网关层（`app/llm/adapter.py`），稳定抵御500/502等偶发网络瞬连波动。
