...
Shared building blocks are defined first, then composed into full prompts.
To tweak any prompt, edit this file — changes are isolated from runtime config.
"""
...
NOVEL_FIDELITY_RULES = """## 内容真实性约束（最高优先级，违反视为失败）
1. **主线严格**：本章所有主线事件、关键决策、命名 NPC 的关键对话与行动，
   必须能在下方"事件台账（event_ledger）"中找到对应原文。
   台账中没有出现的剧情转折、命名 NPC 的关键行为、关键战斗结果，一律禁止编造。
2. **润色自由**：可以为台账事件补写：环境描写（视/听/嗅）、主角心理活动、
   过场段落、未具名 NPC（无名士兵、路人）的简短反应、合理的内心独白与回忆闪回。
3. **台账缺失时的处理**：若台账中某段描述语焉不详，用模糊化、留白、闪回带过，
   禁止自行补足具体剧情细节。
4. **角色一致性**：所有命名角色的言行必须与"角色档案"一致；
   不要给他们安排台账之外的关键决定或未记录的对话内容。"""
...
## 物品栏变化
当场景中主角获得、失去或使用物品时，返回 inventory_changes 数组：
[{{{{"op": "add"|"remove"|"update", "item_name": "物品名", "item_type": "weapon|armor|consumable|key|misc", "qty": 数量, "effects": ["效果描述"], "description": "物品说明（可选）"}}}}]

物品 stats 字段（仅 weapon/armor 类型在 op="add" 时填写）：
- attack_bonus: 攻击伤害加成（整数，1-5，仅 weapon）
- check_bonus: 检定属性加成，如 {{"strength": 2, "dexterity": 1}}（仅 weapon/armor）
- hp_bonus: 最大 HP 提升（整数，1-10，仅 armor）
- defense: 伤害减免值（整数，1-3，仅 armor）

示例：[{{{{"op":"add","item_name":"秘银长剑","item_type":"weapon","qty":1,"effects":["攻击+2"],"description":"削铁如泥","stats":{{"attack_bonus":2,"check_bonus":{{"strength":1}}}}}}}}]
只在确实有物品变化时返回；不要凭空为角色制造物品。

## 任务目标
当剧情中出现新的任务目标、目标完成或失败时，返回 objectives 数组：
[{{{{"id": "任务唯一标识", "title": "目标描述（≤30字）", "status": "active|done|failed"}}}}]
已有任务更新时 id 必须与之前保持一致；不必每轮返回，只在目标有变化时返回。
...
## 关键事件（永久保留的核心锚点，由系统维护）
（**重要：此段落内容由系统自动维护，你只需要原样保留已有内容，不要改写、不要删除、不要新增此段落的条目。**）
...
## 故事流程
（**重要：你只需在本段末尾追加本回合的一行记录，格式严格如下：**
"第{{current_round}}轮：[场景一句话 ≤30字] → 玩家选择「[selected_choice]」→ [检定结果或结果一句话]"
**绝对禁止删除或改写已有轮次条目**——系统将在你返回后自动校验并补全，若已有轮次缺失视为错误。）
（**压缩规则（仅限整体条目过多时）**：当流程超过 20 条时，允许把最早的连续 5-8 条合并为"第X-Y轮：……"概述，但只能合并 **原有** 旧条目，本轮不可被合并。）
...
2. 必须保留"世界观设定"段原文不变。
3. **必须原样保留"关键事件"段落中的已有内容**，不要增删改。
4. 关系变化要在"主要角色"段对应条目里反映出来；新出场的 NPC 要新增条目。
5. 路线得分**不要**写进 memory，仅作为你判断"故事概要"基调与"未解决伏笔"走向的依据。
...
NOVEL_TITLE_PROMPT = f"""你是一个专业的小说策划师。请根据以下真实游戏事件台账与记忆文档，为小说创作总标题与章节大纲。

## 权威事件台账（按游戏实际进行顺序，一轮一条）
这是玩家真实走过的每一轮剧情，章节划分**必须严格以此为依据**，不允许规划台账之外的剧情。
{{event_ledger_overview}}

## 游戏记忆文档（提供世界观与软状态参考）
{{memory_content}}

## 章节规划要求
- 小说应有 {{min_chapters}} 到 {{max_chapters}} 个章节（按台账事件密度划分，不要均匀切分）
- 每章约 2000 字
- 章节总量不能超过游戏总轮数的 50%，不能少于 30%

## 硬性规则
1. 总标题要简短有力（≤14 字），有节奏感与冲突感，避免平淡的"XX的冒险"。
2. 每章 summary 写 100-150 字，必须包含：本章核心事件（来自台账）、关键人物、戏剧冲突、悬念钩子。
3. **summary 中描述的所有事件必须能在台账中找到对应轮次**；不允许在 summary 里预先安排台账中没有的剧情转折。
4. 章节标题彼此呼应、有进度感（开端 → 升级 → 转折 → 高潮）。
5. 章节之间不要内容重复，每章要有明确推进。
...
{{{{
  ...
  "chapters": [
    {{{{
      "chapter_num": 1,
      "title": "章节标题（≤12字）",
      "summary": "本章 100-150 字概要（仅引用台账事件）"
    }}}}
  ]
}}}}
...
## 本章权威事件台账（玩家真实走过的轮次，必须逐条落实到正文）
以下是本章必须覆盖的真实游戏事件。每一个【第N轮】条目都代表真实发生的一轮，**必须**在正文中有对应场景；
"玩家选择"是主角实际做出的选择，必须作为本章关键决策点；
"结果概述"是该轮的实际结果，不允许改变。
{{chapter_event_ledger}}
...
5. 必须把"权威事件台账"中每一轮的场景/选择/结果扩展为完整场景，**不允许**跳过或改变结果；可以补充感官细节、心理描写、环境渲染。
6. 台账之外不允许安排命名 NPC 的关键行动或未记录的剧情转折。
7. 人物的性格、口吻、外貌特征必须严格符合"角色档案"，不要凭空改设定。
8. 不要在章末写"预知后事如何"等评书腔。
...
## 全程权威事件台账（玩家真实选择的完整记录，终章必须以此为骨架）
终章描述的所有具体情节必须锚定在以下台账记录中；
结局的走向由"主导路线"决定，但具体发生的事件不能超出台账记载。
{{final_rounds_ledger}}
...
## 结局与路线信息
- 用户选择的结局类型：{{ending_type}}
- 用户期望的结局细节：{{custom_description}}
- 主导路线：{{route_leader}}
- 各路线得分：{{route_scores}}
...
1. 字数 1800-2200 字。
2. 章节开头用 `## 终章` 标记。
3. **终章的核心情节必须深度融合用户选择的结局类型（{{ending_type}}）以及期望的结局细节（{{custom_description}}）**。如果有自定义细节描述，必须根据用户的期望来展开结局。
4. 终章基调需与"主导路线"对应作为补充色调：
...
5. 必须呼应至少 2 个"未解决伏笔"中列出的线索；其余可以留白但不能与设定矛盾。
6. 全文第三人称全知视角，与正文保持一致。
7. 不要再设置新的悬念或开放式追问。
...
NOVEL_INCREMENTAL_PLAN_PROMPT = f"""你是一个专业的小说策划师。请根据以下真实事件台账为小说规划续写章节。

## 小说标题
{{novel_title}}

## 已有章节概况（共 {{existing_chapters_count}} 章，已覆盖第 1-{{last_covered_round}} 轮）
{{existing_chapters_summary}}

## 续写范围：第 {{last_covered_round}} 轮之后的新增事件台账
以下是玩家真实走过但尚未被小说覆盖的每一轮记录，续写章节**必须严格以此为划分依据**，不允许规划台账之外的剧情。
{{event_ledger_overview}}
...
## 续写章节规划要求
- 新增章节数量 {{min_chapters}} 到 {{max_chapters}} 章
- 章节编号从第 {{start_chapter_num}} 章开始（接续已有章节）
- 每章约 2000 字

## 硬性规则
1. 续写章节必须与已有章节保持情节衔接和风格一致。
2. **严禁**重写或改动已覆盖（第 1-{{last_covered_round}} 轮）的事件；可以用闪回/回忆带过，但不能与已有章节矛盾。
3. 每章 summary 写 100-150 字，必须包含：本章核心事件（引用台账轮次）、关键人物、戏剧冲突；**summary 描述的事件必须在台账中可找到对应**。
4. 章节标题要延续已有章节的命名风格。
5. 如果台账条目稀少，允许在同一章内覆盖多轮，但不允许跨越台账编造中间剧情。
